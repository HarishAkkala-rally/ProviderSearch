 ...
 */
  ...
  def encodeAuthSessionCookie(
    rallyId: String
  ): Map[String, String] = {
    val (sessionId, session) = AuthUtils.newUserSession(rallyId)
    val authUtils = new AuthUtils()
    val xsrfCookie: Cookie = authUtils.newXSRFCookie(sessionId)
    val sessionCookie: Cookie = new LegacySessionCookieBaker(config, cookieSigner).encodeAsCookie(session)
//    val sessionCookie: Cookie = new DefaultSessionCookieBaker(config, secret, cookieSigner).encodeAsCookie(session)   //SIGNING ISSUE
    Map(
      COOKIE -> new DefaultCookieHeaderEncoding().encodeCookieHeader(Seq(sessionCookie, xsrfCookie)),
      AuthServiceConfig.XSRFHeader -> xsrfCookie.value
    )
  }
...
//
//  def generateCookie = {
////    new DefaultSessionCookieBaker(config, secret, cookieSigner).encodeAsCookie(Session(Map("created" -> "", "heartbeat" -> "", "id" -> "0414ea4a-9df0-4976-9797-7f9b52deee9e", "sid" -> randomUuid)))
